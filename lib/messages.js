...
function Version(subversion, nonce) {
  ...
  this.subversion = subversion || '/faircore:' + packageInfo.version + '/';
...
Version.prototype.fromBuffer = function(payload) {
   ...
   */
  this.addr_me = parser.read(30);
  /**
   * @type {Buffer}
   * @desc IPv4/6 address of the peer
   */
  this.addr_you = parser.read(30);
  /**
   * @type {Buffer}
 ...
 * Checkpoint Message
 *
 * @name P2P.Message.Checkpoint
 * @param{Buffer} checkpoint - reported elements
 */
function Checkpoint(checkpoint) {
  this.command = 'checkpoint';
  this.checkpoint = checkpoint || {};
}
util.inherits(Checkpoint, Message);

Checkpoint.prototype.fromBuffer = function(payload) {
  var parser = new BufferReader(payload);
  this.checkpoint = {
      version: parser.readUInt32LE(),
      hash: parser.read(32),
      signature: parser.read(70)
  };

  return this;
};

Checkpoint.prototype.getPayload = function() {
  var put = new Put();

  put.word32le(this.checkpoint.version);
  put.put(this.checkpoint.hash);
  put.put(this.checkpoint.signature);

  return put.buffer();
};
  
module.exports.Checkpoint = Message.COMMANDS.checkpoint = Checkpoint;
